...
import org.whu.database.SQLiteDatabaseConfiguration;
import org.whu.database.SQLiteDebug;
import org.whu.database.SQLiteGlobal;
...
public final class SQLiteConnectionPool implements Closeable {
    ...
    private final org.whu.database.CloseGuard mCloseGuard = org.whu.database.CloseGuard.get();
    ...
    private final ArrayList<org.whu.database.SQLiteConnection> mAvailableNonPrimaryConnections = new ArrayList<>();
    private org.whu.database.SQLiteConnection mAvailablePrimaryConnection;
    ...
    private final WeakHashMap<org.whu.database.SQLiteConnection, AcquiredConnectionStatus> mAcquiredConnections =
            new WeakHashMap<>();
    ...
    public void reconfigure(SQLiteDatabaseConfiguration configuration) {
        ...
        synchronized (mLock) {
            ...
            if (mConfiguration.openFlags != configuration.openFlags) {
                ...
                org.whu.database.SQLiteConnection newPrimaryConnection = openConnectionLocked(configuration,
                        true /*primaryConnection*/); // might throw
     ...
     */
    public org.whu.database.SQLiteConnection acquireConnection(String sql, int connectionFlags,
     ...
     */
    public void releaseConnection(org.whu.database.SQLiteConnection connection) {
        synchronized (mLock) {
    ...
    // Can't throw.
    private boolean recycleConnectionLocked(org.whu.database.SQLiteConnection connection,
     ...
     * from this pool or if it has already been released.
     */
    public boolean shouldYieldConnection(org.whu.database.SQLiteConnection connection, int connectionFlags) {
        synchronized (mLock) {
    ...
    public void collectDbStats(ArrayList<SQLiteDebug.DbStats> dbStatsList) {
        synchronized (mLock) {
            ...
            for (org.whu.database.SQLiteConnection connection : mAvailableNonPrimaryConnections) {
                connection.collectDbStats(dbStatsList);
            }

            for (org.whu.database.SQLiteConnection connection : mAcquiredConnections.keySet()) {
    ...
    // Might throw.
    private org.whu.database.SQLiteConnection openConnectionLocked(SQLiteDatabaseConfiguration configuration,
                                                                   boolean primaryConnection) {
        final int connectionId = mNextConnectionId++;
        return org.whu.database.SQLiteConnection.open(this, configuration,
    ...
    private void closeExcessConnectionsAndLogExceptionsLocked() {
        ...
        while (availableCount-- > mMaxConnectionPoolSize - 1) {
            org.whu.database.SQLiteConnection connection =
    ...
    // Can't throw.
    private void closeConnectionAndLogExceptionsLocked(org.whu.database.SQLiteConnection connection) {
        try {
    ...
    private void reconfigureAllConnectionsLocked() {
        ...
        for (int i = 0; i < count; i++) {
            final org.whu.database.SQLiteConnection connection = mAvailableNonPrimaryConnections.get(i);
            try {
                connection.reconfigure(mConfiguration); // might throw
            } catch (RuntimeException ex) {
    ...
    private void markAcquiredConnectionsLocked(AcquiredConnectionStatus status) {
        if (!mAcquiredConnections.isEmpty()) {
            ArrayList<org.whu.database.SQLiteConnection> keysToUpdate = new ArrayList<>(
                    mAcquiredConnections.size());
            for (Map.Entry<org.whu.database.SQLiteConnection, AcquiredConnectionStatus> entry
            ...
            for (org.whu.database.SQLiteConnection key : keysToUpdate) {
    ...
    // Might throw.
    private org.whu.database.SQLiteConnection waitForConnection(String sql, int connectionFlags,
                                                                CancellationSignal cancellationSignal) {
        ...
        synchronized (mLock) {
            ...
            org.whu.database.SQLiteConnection connection = null;
        ...
        try {
            ...
            for (;;) {
                ...
                synchronized (mLock) {
                    ...
                    final org.whu.database.SQLiteConnection connection = waiter.mAssignedConnection;
    ...
    // Might throw.
    private org.whu.database.SQLiteConnection tryAcquirePrimaryConnectionLocked(int connectionFlags) {
        // If the primary connection is available, acquire it now.
        org.whu.database.SQLiteConnection connection = mAvailablePrimaryConnection;
        if (connection != null) {
            mAvailablePrimaryConnection = null;
        ...
        for (org.whu.database.SQLiteConnection acquiredConnection : mAcquiredConnections.keySet()) {
    ...
    // Might throw.
    private org.whu.database.SQLiteConnection tryAcquireNonPrimaryConnectionLocked(
    ...
    // Might throw.
    private void finishAcquireConnectionLocked(org.whu.database.SQLiteConnection connection, int connectionFlags) {
        try {
    ...
    public void dump(Printer printer, boolean verbose) {
        synchronized (mLock) {
            ...
            if (!mAcquiredConnections.isEmpty()) {
                for (Map.Entry<org.whu.database.SQLiteConnection, AcquiredConnectionStatus> entry :
                        mAcquiredConnections.entrySet()) {
                    final org.whu.database.SQLiteConnection connection = entry.getKey();
    ...
    private static final class ConnectionWaiter {
        ...
        public org.whu.database.SQLiteConnection mAssignedConnection;
