...
/* creating a further ordinary Column */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("ALTER TABLE ADD COLUMN error: %s\n", err_msg);
...
/* creating a further Geometry Column */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("ADD GEOMETRY COLUMN error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
    ...
    if (column->geometry->spatial_index)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("CREATE SPATIAL INDEX error: %s\n", err_msg);
		sqlite3_free (err_msg);
		return 0;
	    }
      }
...
/* creating a Cloner object */
    ...
    if (checkSpatialMetaData ((sqlite3 *) sqlite) < 3)
      {
	  whu_e ("CloneTable: obsolete DB-layout (< 4.0.0)\n");
	  return NULL;
      }
...
/* executing the actual work */
    ...
    if (!copy_rows (cloner))
      {
	  whu_e ("CloneTable: unable to copy Table rows\n");
	  return 0;
      }
    return 1;
}
