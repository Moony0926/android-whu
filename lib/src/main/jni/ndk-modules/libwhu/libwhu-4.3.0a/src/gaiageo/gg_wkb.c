...
{
/* decodes a MULTIxx or GEOMETRYCOLLECTION from Whu BLOB */
    int entities;
    int type;
    int ie;
    if (geo->size < geo->offset + 4)
...
gaiaFromWhuBlobWkbEx (const unsigned char *blob, unsigned int size,
			     int gpkg_mode, int gpkg_amphibious)
{
/* decoding from Whu BLOB to GEOMETRY */
    int type;
    int little_endian;
...
GAIAGEO_DECLARE gaiaGeomCollPtr
gaiaFromWhuBlobWkb (const unsigned char *blob, unsigned int size)
{
/* 
* decoding from Whu BLOB to GEOMETRY
* convenience method - always disabling GPKG compatibility Modes
*/
    return gaiaFromWhuBlobWkbEx (blob, size, 0, 0);
}

GAIAGEO_DECLARE gaiaGeomCollPtr
gaiaFromWhuBlobMbr (const unsigned char *blob, unsigned int size)
{
/* decoding from Whu BLOB to GEOMETRY [MBR only] */
    int little_endian;
    int endian_arch = gaiaEndianArch ();
    double minx;
    double miny;
...
gaiaToWhuBlobWkbEx (gaiaGeomCollPtr geom, unsigned char **result,
			   int *size, int gpkg_mode)
{
/* builds the Whu BLOB representation for this GEOMETRY */
    int ib;
    int iv;
    double x;
    double y;
...
GAIAGEO_DECLARE void
gaiaToWhuBlobWkb (gaiaGeomCollPtr geom, unsigned char **result,
			 int *size)
{
/* 
* builds the Whu BLOB representation for this GEOMETRY
* convenience method - always disabling GPKG compatibility Modes
*/
    gaiaToWhuBlobWkbEx (geom, result, size, 0);
}
