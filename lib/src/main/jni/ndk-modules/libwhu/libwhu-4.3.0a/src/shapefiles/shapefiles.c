...
/* checking if TABLE already exists */
    ...
    if (already_exists)
      {
	  if (!err_msg)
	      whu_e ("load shapefile error: table '%s' already exists\n",
			    table);
	  else
	      sprintf (err_msg,
    ...
    while (1)
	  ...
	  if (shp->Dbf->Geometry)
		...
		if (compressed)
		    ...
		    gaiaToWhuBlobWkb (shp->Dbf->Geometry, &blob,
					     &blob_size);
...
/* step I: retrieving the SRID */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("dump shapefile MetaData error: <%s>\n", errMsg);
	  sqlite3_free (errMsg);
	  return;
      }
    for (i = 1; i <= rows; i++)
...
/ 1 - if Whu-like (legacy) SpatialMetadata were found
/ 2 - if FDO-OGR-like SpatialMetadata were found
/ 3 - if Whu-like (current) SpatialMetadata were found
/
*/
...
/* verifying the MetaData format */
    if (whu_legacy_gc && whu_legacy_rs)
	return 1;
    if (fdo_gc && fdo_rs)
	return 2;
    if (whu_gc && whu_rs)
...
/* some SQL error occurred */
	...
	gaiaFreeShapefile (shp);
    if (!err_msg)
	whu_e ("SELECT failed: %s", sqlite3_errmsg (sqlite));
    else
...
/* shapefile can't be created/opened */
	...
	gaiaFreeShapefile (shp);
    if (!err_msg)
	whu_e ("ERROR: unable to open '%s' for writing", shp_path);
    else
...
/* checking if TABLE already exists */
    ...
    if (already_exists)
      {
	  if (!err_msg)
	      whu_e ("load DBF error: table '%s' already exists\n",
			    table);
	  else
    ...
    if (!(dbf->Valid))
	  ...
	  if (!err_msg)
	    {
		whu_e ("load DBF error: cannot open '%s'\n", dbf_path);
		if (dbf->LastError)
		    whu_e ("\tcause: %s\n", dbf->LastError);
	    }
	  else
...
/* xfering export-list ownership */
    ...
    free (xtable);
    if (!err_msg)
	whu_e ("Exported %d rows into the DBF file\n", rows);
    else
...
/* DBF can't be created/opened */
	...
	gaiaFreeDbf (dbf);
    if (!err_msg)
	whu_e ("ERROR: unable to open '%s' for writing\n", dbf_path);
    else
...
/* extracting the column names (excluding any Primary Key) */
    ...
    while (1)
	  ...
	  if (ret == SQLITE_ROW)
		...
		*dupl_count += sqlite3_column_int (stmt, 0) - 1;
	    ...
	    {
		whu_e ("SQL error: %s", sqlite3_errmsg (sqlite));
		sqlite3_finalize (stmt);
		return;
	    }
      }
    sqlite3_finalize (stmt);
    if (*dupl_count)
	whu_e ("%d duplicated rows found !!!\n", *dupl_count);
    else
	whu_e ("No duplicated rows have been identified\n");
}
    ...
    if (do_delete_duplicates (sqlite, sql, sql2, &count, transaction))
	  ...
	  if (removed == NULL)
		...
		if (!count)
		    ...
		    whu_e ("%d duplicated rows deleted from: %s\n",
				  count, table);
	    }
	  else
	      *removed = count;
...
/* data transfer */
    ...
    while (1)
	  ...
	  if (ret == SQLITE_ROW)
	    {
		gaiaGeomCollPtr g =
		    gaiaFromWhuBlobWkb ((const unsigned char *)
...
/* checking if TABLE already exists */
    ...
    if (already_exists)
      {
	  if (!err_msg)
	      whu_e ("load XL error: table '%s' already exists\n",
			    table);
	  else
...
/* preparing the INSERT INTO parameterized statement */
  ...
  error:
    freexl_close (xl_handle);
    if (!err_msg)
	whu_e ("XL datasource '%s' is not valid\n", path);
    else
...
/* the result set is empty - nothing to do */
    ...
    whu_e ("The SQL SELECT returned no data to export...\n");
    return 0;
}
