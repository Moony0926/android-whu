...
/* adding the geometry column */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("gpkgAddGeometryColumn \"%s\" error: %s\n", table_name,
			sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
...
/* adding the geometry triggers */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("gpkgAddGeometryTriggers \"%s\" error: %s\n",
			table_name, sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
    ...
    if (spatial_index)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("gpkgAddSpatialIndex \"%s\" error: %s\n",
			      table_name, sql_err);
		sqlite3_free (sql_err);
		return 0;
	    }
...
static int
create_whu_destination (sqlite3 * handle, const char *create_sql,
			       const char *table_name, const char *column_name,
			       const char *geometry_type, const char *dims,
			       int srid)
{
/* attempting to create a Whu destination table */
    int ret;
    char *sql_err = NULL;
    char *sql;
    int i;
...
/* adding the geometry column */
    ...
    if (!ok)
      {
	  whu_e ("AddGeometryColumn \"%s\": error\n", table_name);
	  return 0;
      }
    return 1;
}
...
static int
create_Whu2GPKG_statements (sqlite3 * handle_in, sqlite3 * handle_out,
...
/* starting a transaction */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("BEGIN TRANSACTION error: %s\n", sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
...
static int
create_GPKG2Whu_statements (sqlite3 * handle_in, sqlite3 * handle_out,
				   const char *table_name,
				   const char *geometry_column,
				   sqlite3_stmt ** stmt_in,
				   sqlite3_stmt ** stmt_out)
	  ...
	  if (ret == SQLITE_ROW)
		...
		if (ret == SQLITE_DONE || ret == SQLITE_ROW)
		...
		else
		  {
		      ...
		      whu_e ("Error while inserting into \"%s\": %s\n",
	    ...
	    {
		/* an unexpected error occurred */
		whu_e ("Error while querying from \"%s\": %s\n",
...
/* committing the still pending transaction */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("COMMIT TRANSACTION error: %s\n", sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
    return 1;
...
/* invalidating the still pending transaction */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("ROLLBACK TRANSACTION error: %s\n", sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
    return 0;
...
/* registering the GPKG table in GPKG_CONTENTS */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("INSERT INTO gpkg_contents error: %s\n", sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
    return 1;
...
static int
copy_whu2GPKG (sqlite3 * handle_in, sqlite3 * handle_out, int legacy)
{
...
static int
copy_GPKG2Whu (sqlite3 * handle_in, sqlite3 * handle_out)
{
/* attempting to copy all Geometry Tables */
    int ret;
    const char *sql;
...
static int
check_valid_whu (sqlite3 * handle, int *legacy)
{
/* checking for a valid Whu origin */
    int ret;
    int i;
    char **results;
    int rows;
...
/* creating Metadata tables */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("gpkgCreateBaseTables error: %s\n", sql_err);
	  sqlite3_free (sql_err);
	  return 0;
      }
    return 1;
...
static int
initialize_whu (sqlite3 * handle)
{
/* attempting to initialize a Whu destination */
    int ret;
    int i;
    char **results;
    int rows;
...
WHU_DECLARE int
gaiaGPKG2Whu (sqlite3 * handle_in, const char *gpkg_in_path,
		     sqlite3 * handle_out, const char *splite_out_path)
{
/* attempting to create a Whu DB by converting a GPKG DB */
    if (handle_in == NULL)
	goto error;
    if (!check_valid_gpkg (handle_in))
      {
	  /* not a valid IN GPKG: quitting */
	  whu_e ("Invalid GPKG origin:\n\"%s\"\n", gpkg_in_path);
	  goto error;
      }
    if (handle_out == NULL)
	goto error;
    if (!initialize_whu (handle_out))
      {
	  /* unable to initialize a valid Whu DB: quitting */
	  whu_e
	      ("Unable to initialize Whu destination:\n\"%s\"\n",
	       splite_out_path);
	  goto error;
      }

/* copying/converting all Spatial tables */
    if (!copy_GPKG2Whu (handle_in, handle_out))
	goto error;
    return 1;

  error:
    whu_e ("Conversion aborted due to previous error(s)\n");
    return 0;
}

WHU_DECLARE int
gaiaWhu2GPKG (sqlite3 * handle_in, const char *splite_in_path,
		     sqlite3 * handle_out, const char *gpkg_out_path)
{
/* attempting to create a GPKG DB by converting a Whu DB */
    int legacy;

    if (handle_in == NULL)
	goto error;
    if (!check_valid_whu (handle_in, &legacy))
      {
	  /* not a valid IN GPKG: quitting */
	  whu_e ("Invalid Whu origin:\n\"%s\"\n", splite_in_path);
	  goto error;
      }
    if (handle_out == NULL)
	goto error;
    if (!initialize_gpkg (handle_out))
	  ...
	  whu_e ("Unable to initialize GPKG destination:\n\"%s\"\n",
			gpkg_out_path);
	  goto error;
      }

/* copying/converting all Spatial tables */
    if (!copy_whu2GPKG (handle_in, handle_out, legacy))
	goto error;
    return 1;
