...
/* auxiliary - updating "splite_metacatalog_statistics" */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Update MetaCatalog Statistics(4) error: \"%s\"\n",
...
/* deleting all existing rows */
	...
	;
      ...
      {
	  whu_e ("populate MetaCatalog Statistics(5) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
...
/* updating the MetaCatalog statistics */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Update MetaCatalog Statistics(1) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
    ...
    if (ret != SQLITE_OK)
      {
	  sqlite3_finalize (stmt_in);
	  whu_e ("Update MetaCatalog Statistics(2) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
    ...
    if (ret != SQLITE_OK)
	  ...
	  whu_e ("Update MetaCatalog Statistics(3) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
...
/* checks if a column has any Unique constraint - pass two */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("populate MetaCatalog(8) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
...
/* checks if a column has any Unique constraint */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("populate MetaCatalog(7) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
...
/* checks if a column is part of any Foreign Key */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("populate MetaCatalog(6) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
...
/* auxiliary - populating "splite_metacatalog" */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("populate MetaCatalog(3) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
    ...
    while (1)
	  ...
	  if (ret == SQLITE_ROW)
		...
		if (ret == SQLITE_DONE || ret == SQLITE_ROW)
		  ...
		  {
		      whu_e ("populate MetaCatalog(4) error: \"%s\"\n",
				    sqlite3_errmsg (sqlite));
		      sqlite3_finalize (stmt_in);
		      return 0;
		  }
...
/* populating the MetaCatalog table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("populate MetaCatalog(1) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
    ...
    if (ret != SQLITE_OK)
      {
	  sqlite3_finalize (stmt_in);
	  whu_e ("populate MetaCatalog(2) error: \"%s\"\n",
			sqlite3_errmsg (sqlite));
	  return 0;
      }
...
/* creating the "raster_coverages" table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'raster_coverages' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the raster_coverages_srid table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'raster_coverages_srid' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the raster_coverages_ref_sys view */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE VIEW 'raster_coverages_ref_sys' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the raster_coverages_keyword table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'raster_coverages_keyword' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the "vector_coverages" table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'vector_coverages' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
/* creating the VectorLayers index */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE INDEX 'idx_vector_coverages' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the vector_coverages_srid table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'vector_coverages_srid' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the vector_coverages_ref_sys view */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE VIEW 'vector_coverages_ref_sys' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the vector_coverages_keyword table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'vector_coverages_keyword' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_external_graphics table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_external_graphics' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_fonts table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_fonts' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_vector_styles table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_vector_styles' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
/* creating the style Name index */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE INDEX 'idx_vector_styles' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_vector_styled_layers table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_vector_styled_layers' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
/* creating the style_id index */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE INDEX 'idx_svstl_style' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_raster_styles table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_raster_styles' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
/* creating the style Name index */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE INDEX 'idx_raster_styles' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_raster_styled_layers table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_raster_styled_layers' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
/* creating the style_id index */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE INDEX 'idx_serstl_style' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_group_styles table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_group_styles' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_styled_groups table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'SE_styled_groups' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating any Index on SE_styled_group_refs */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Create Index 'idx_SE_styled_vgroups' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Create Index 'idx_SE_styled_rgroups' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the SE_fonts_view view */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE VIEW 'SE_fonts_view' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
    return 1;
...
/* creating the ISO_metadata table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'ISO_metadata' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating any Index on ISO_metadata */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Create Index 'idx_ISO_metadata_ids' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("Create Index 'idx_ISO_metadata_parents' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the ISO_metadata_reference table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE TABLE 'ISO_metadata_reference' error: %s\n",
			err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
...
/* creating the ISO_metadata_view view */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE VIEW 'ISO_metadata_view' error: %s\n", err_msg);
	  sqlite3_free (err_msg);
	  return 0;
      }
    return 1;
