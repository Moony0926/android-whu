...
static int
testWhuHistory (sqlite3 * sqlite)
{
/* internal utility function:
/
/ checks if the WHU_HISTORY table already exists
/
*/
    int event_id = 0;
    int table_name = 0;
    ...
    int rows;
    int columns;
/* checking the WHU_HISTORY table */
    strcpy (sql, "PRAGMA table_info(whu_history)");
...
static int
checkWhuHistory (sqlite3 * sqlite)
{
/* internal utility function:
/
/ checks if the WHU_HISTORY table already exists
...
*/
    ...
    if (testWhuHistory (sqlite))
	return 1;

/* creating the WHU_HISTORY table */
    strcpy (sql, "CREATE TABLE IF NOT EXISTS ");
    strcat (sql, "whu_history (\n");
...
WHU_PRIVATE void
updateWhuHistory (void *p_sqlite, const char *table,
			 const char *geom, const char *operation)
{
/* inserting a row in WHU_HISTORY */
    sqlite3 *sqlite = (sqlite3 *) p_sqlite;
    char sql[2048];
    sqlite3_stmt *stmt = NULL;
    int ret;

    if (checkWhuHistory (sqlite) == 0)
	return;

    strcpy (sql, "INSERT INTO whu_history ");
    ...
    strcat (sql, "sqlite_version(), whu_version())");
...
/* creating the GEOMETRY_COLUMNS table */
    ...
    updateWhuHistory (sqlite, "geometry_columns", NULL,
...
/* compiling SQL prepared statement */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("upgradeGeometryTriggers: error %d \"%s\"\n",
...
/* compiling SQL prepared statement */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("updateTableTriggers: error %d \"%s\"\n",
			sqlite3_errcode (sqlite), sqlite3_errmsg (sqlite));
	  return;
      }
    sqlite3_reset (stmt);
...
/* loading a SpatialIndex [RTree] */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("buildSpatialIndex error: \"%s\"\n", errMsg);
	  sqlite3_free (errMsg);
	  return -1;
      }
    return 0;
...
WHU_DECLARE gaiaVectorLayersListPtr
