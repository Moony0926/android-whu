		...
		if (geom == NULL)
		...
		else
		  {
			  ...
			  && schema->geometry_type == GAIA_MULTIPOLYGON)
			{
			  ...
			  gaiaSwapCoords (geom);
		      gaiaToWhuBlobWkb (geom, &blob, &blob_size);
...
/* attempting to create the SQL Table */
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("loadwfs: CREATE TABLE '%s' error: %s\n", table,
			errMsg);
	  schema->error = 1;
	  if (err_msg == NULL)
	    {
    ...
    if (schema->geometry_name != NULL)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("loadwfs: AddGeometryColumn error: %s\n", errMsg);
		schema->error = 1;
		if (err_msg == NULL)
		  {
		      sqlite3_free (errMsg);
	  ...
	  if (spatial_index)
		...
		if (ret != SQLITE_OK)
		  {
		      whu_e ("loadwfs: CreateSpatialIndex error: %s\n",
				    errMsg);
		      schema->error = 1;
		      if (err_msg == NULL)
			{
...
/* invalidating the still pending SQL transaction */
    ...
    if (sqlite3_exec (sqlite, "ROLLBACK", NULL, NULL, &errMsg) != SQLITE_OK)
      {
	  whu_e ("loadwfs: ROLLBACK error:\"%s\"\n", errMsg);
	  sqlite3_free (errMsg);
	  schema->error = 1;
      }
}
...
/* starting a transaction */
    ...
    if (cast_type || cast_dims)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("loadwfs: UPDATE-GEOMS error:\"%s\"\n", errMsg);
		sqlite3_free (errMsg);
	    }
      }
...
/* LIBXML2 isn't enabled: always returning an error */
    int len;
    const char *msg = "Sorry ... libwhu was built disabling LIBXML2\n"
