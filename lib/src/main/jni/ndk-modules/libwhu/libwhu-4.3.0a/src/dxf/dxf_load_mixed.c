...
/* attempting to create the "Text-mixed" table */
    ...
    *xstmt = NULL;
    ...
    if (ret != SQLITE_OK)
      {
	  whu_e ("CREATE SPATIAL INDEX %s error: %s\n", name,
			sqlite3_errmsg (handle));
	  return 0;
      }
...
/* populating the target DB - all layers mixed altogether */
    ...
    if (text)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("COMMIT text_layer error: %s\n",
			      sqlite3_errmsg (handle));
		return 0;
	    }
      }
    ...
    if (point)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("COMMIT point_layer error: %s\n",
			      sqlite3_errmsg (handle));
		return 0;
	    }
      }
    ...
    if (line)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("COMMIT line_layer error: %s\n",
			      sqlite3_errmsg (handle));
		return 0;
	    }
      }
    ...
    if (polyg)
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("COMMIT polyg_layer error: %s\n",
			      sqlite3_errmsg (handle));
		return 0;
	    }
      }
    ...
    if (hatch)
		...
		while (hatch != NULL)
		  {
			...
			{
			    gaiaToWhuBlobWkb (hatch->boundary, &blob,
						     &blob_size);
			    sqlite3_bind_blob (stmt, 3, blob, blob_size, free);
			}
		      ret = sqlite3_step (stmt);
	  ...
	  if (ret != SQLITE_OK)
	    {
		whu_e ("COMMIT hatch_layer error: %s\n",
			      sqlite3_errmsg (handle));
		return 0;
	    }
      }
