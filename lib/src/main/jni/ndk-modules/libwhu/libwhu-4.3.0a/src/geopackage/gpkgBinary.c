/*
    ...
    GeoPackage extensions for Whu / SQLite
...
/
/ converts the (whu) geometry blob into a GeoPackage format geometry blob
...
*/
    ...
    geo = gaiaFromWhuBlobWkb (p_blob, n_bytes);
    if (!geo)
      {
	  sqlite3_result_null (context);
	  return;
...
*/
    ...
    gaiaToWhuBlobWkb (geo, &p_result, &len);
    gaiaFreeGeomColl (geo);
    sqlite3_result_blob (context, p_result, len, free);
}
