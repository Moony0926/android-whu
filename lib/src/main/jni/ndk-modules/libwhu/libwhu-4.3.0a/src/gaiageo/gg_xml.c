...
/* testing if the passed cache is a valid one */
    if (cache == NULL)
	return 0;
    if (cache->magic1 != WHU_CACHE_MAGIC1
	|| cache->magic2 != WHU_CACHE_MAGIC2)
	return 0;
    return 1;
}
...
/* attempting to retrieve the Geometry item */
    ...
    if (geom)
	  ...
	  gaiaToWhuBlobWkb (geom, &blob, &blob_len);
	...
	*schema_validation_errors = NULL;
    ...
    if (schemaURI != NULL)
	  ...
	  if (splite_xmlSchemaCacheFind
		...
		if (schema_doc == NULL)
		  {
		      whu_e ("unable to load the Schema\n");
		      if (schema_validation_errors)
		...
		if (parser_ctxt == NULL)
		  {
		      whu_e ("unable to prepare the Schema Context\n");
...
/* testing if the XMLDocument is well-formed */
	...
	*parsing_errors = parsingBuf->Buffer;
    ...
    if (schemaURI != NULL)
	  ...
	  if (valid_ctxt == NULL)
	    {
		whu_e ("unable to prepare a validation context\n");
		xmlFreeDoc (xml_doc);
	  ...
	  if (xmlSchemaValidateDoc (valid_ctxt, xml_doc) != 0)
	    {
		whu_e ("Schema validation failed\n");
...
/* testing for special cases: ISO Metadata, SLD/SE Styles and SVG */
    ...
    if (compressed)
	  ...
	  if (compress (zip_buf, &zLen, (const Bytef *) xml, (uLong) xml_len) !=
	    ...
	    {
		/* compression error */
		whu_e ("XmlBLOB DEFLATE compress error\n");
		free (zip_buf);
		xmlSetGenericErrorFunc ((void *) stderr, NULL);
		return;
	    }
...
/* validity check */
    ...
    if (in_compressed == compressed)
	  ...
	  if (uncompress (xml, &refLen, in, in_zip_len) != Z_OK)
	    {
		/* uncompress error */
		whu_e ("XmlBLOB DEFLATE uncompress error\n");
		free (xml);
		return;
	    }
	  *(xml + in_xml_len) = '\0';
