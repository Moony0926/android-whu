/*
 ...
 whu_init.c -- SQLite3 spatial extension
...
(exposing liblwgeom APIs as Whu own SQL functions)
...
static int
init_whu_extension (sqlite3 * db, char **pzErrMsg, const void *pApi)
{
    if (pApi == NULL)
...
/* setting the POSIX locale for numeric */
    ...
    *pzErrMsg = NULL;
    ...
    register_whu_sql_functions (db, NULL);

    init_whu_virtualtables (db, NULL);
...
*/
    ...
    sqlite3_auto_extension ((void (*)(void)) init_whu_extension);
    whu_splash_screen (verbose);
}
...
{
/* OBSOLETE - strongly discouraged !!!!!
   always using whu_cleanup_ex() as a replacement
   is warmly reccomended
*/
